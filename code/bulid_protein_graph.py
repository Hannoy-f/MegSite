...
def bulid_graph(esm3_fea_path,pdb_path,esm_msa_path,pdb_id,chain_id,thre=14):
    dm,coord_arr,AA_seq=pdbToDistanceMap(pdb_id=pdb_id,chain_id=chain_id,pdb_path=pdb_path)
    adjm=init_adjm_withDM(dm,thre=thre)
    esm_msa_fea=np.loadtxt(esm_msa_path)
    esm3_fea_fea=np.load(esm3_fea_path)
    fea=np.concatenate((esm3_fea_fea,esm_msa_fea),axis=1)
